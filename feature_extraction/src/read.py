...
# from src.util import (# 执行process_data.py
from util import (  # 为了执行read.py
    instance_direction_rect,
    instance_direction_bottom_left,
    save,
    my_range,
    is_gzip_file,
)
...
# 得到了lef文件中所有的宏单元及其尺寸和各个引脚的覆盖区域
def read_lef(path, lef_dict, unit):  # 这里应该是cells.lef
    with open(path, "r") as read_file:
        ...
        for line in read_file:
            if line.lstrip().startswith(
                "MACRO"
            ):  # 读到了宏单元，lef文件的宏单元定义行 形如：MACRO ms00f80
                READ_MACRO = True
            ...
            if READ_MACRO:
                if line.lstrip().startswith("SIZE"):  # SIZE行 形如：SIZE 1.6 BY 2.0 ;
                    l = re.findall(r"-?\d+\.?\d*e?-?\d*?", line)
                    lef_dict[cell_name]["size"] = [
                        unit * float(l[0]),
                        unit * float(l[1]),
                    ]  # size [unit*w,unit*h]
                    ...
                    "PIN"
                ):  # PIN行 形如：  PIN o DIRECTION OUTPUT ;
                    pin_name = line.split()[1]

                elif line.lstrip().startswith("RECT"):  # RECT 0.05 0.500 0.15 1.500 ;
                    l = line.split()
                    rect_list_left.append(float(l[1]))  # 左下角 x
                    rect_list_lower.append(float(l[2]))  # 左下角 y
                    rect_list_right.append(float(l[3]))  # 右上角 x
...
# 得到了lef文件中所有的宏单元及其尺寸和各个引脚的覆盖区域(分层)
def read_lef_pin_map(path, lef_dic, unit):
    with open(path, "r") as read_file:
        ...
        for line in read_file:
            if line.lstrip().startswith("MACRO"):  # MACRO ms00f80
                cell_name = line.split()[1]
                lef_dic[cell_name] = {}
            ...
            if READ_MACRO:
                ...
                if line.lstrip().startswith("SIZE"):  # SIZE 1.6 BY 2.0 ;
                    l = re.findall(r"-?\d+\.?\d*e?-?\d*?", line)
                    lef_dic[cell_name]["size"] = [
                        unit * float(l[0]),
                        unit * float(l[1]),
                    ]

                elif line.lstrip().startswith("PIN") or line.lstrip().startswith(
                    "OBS"
                ):  # PIN o DIRECTION OUTPUT ;
                    if line.lstrip().startswith("OBS"):  # OBS是Obstruction
                        pin_name = "OBS"
                    else:
                        pin_name = line.split()[1]
                    lef_dic[cell_name]["pin"][pin_name] = {}

                elif line.lstrip().startswith("LAYER"):  # LAYER metal2 ;
                    pin_layer = line.split()[1]
                    lef_dic[cell_name]["pin"][pin_name][pin_layer] = []

                elif line.lstrip().startswith("RECT"):
                    l = line.split()
                    lef_dic[cell_name]["pin"][pin_name][pin_layer].append(
                        [
                            float(l[1]) * unit,
                            float(l[2]) * unit,
                            float(l[3]) * unit,
...
class ReadInnovusOutput:
    def __init__(self, root_dir, arg, save_name, lef_dict, lef_dict_jnet=None):
        ...
        self.place_def_path = os.path.join(root_dir, arg.place_def_name)  # 布局def
        self.route_def_path = os.path.join(root_dir, arg.route_def_name)  # 布线def
        self.eGR_congestion_path = os.path.join(
            root_dir, arg.eGR_congestion_name
        )  # 早期全局布线拥塞
        self.route_congestion_path = os.path.join(
            root_dir, arg.route_congestion_name
        )  # 布线拥塞
        self.drc_path = os.path.join(root_dir, arg.drc_rpt_name)  # drc
        ...
        self.gcell_size = [
            -1,
            -1,
        ]  # Number of Gcell grids. # read_route_def()函数中获取
        ...
        self.route_instance_dict = (
            {}
        )  # contains information of instances {inst_name:[std_cell_name, [inst_location_w/h], inst_direction]}
        self.route_net_dict = (
            {}
        )  # contains instance names and pin names in each nets {net_name:[pin_names]}
        self.route_pin_dict = (
            {}
    ...
    def read_place_def(self):
        ...
        for line in read_file:
            ...
            if READ_MACROS:  # get macro_region (routability feature)
                if "FIXED" in line:
                    ...
                    self.place_instance_dict[instance[1]] = [
                        instance[2],
                        coordinate,
                        instance[l + 4],
                    ]
                    (
                        ...
                        line, coordinate, self.lef_dict[instance[2]]["size"]
                    )
                    macro_map[
                        x_y_lower_left_gcell[0] : x_y_upper_right_gcell[0],
                        x_y_lower_left_gcell[1] : x_y_upper_right_gcell[1],
                    ] = np.ones(
                        [
                            x_y_upper_right_gcell[0] - x_y_lower_left_gcell[0],
                            x_y_upper_right_gcell[1] - x_y_lower_left_gcell[1],
                        ]
                    )
                    ...
                    x_y_lower_left = [
                        x_y_lower_left[0] - halo[0],
                        x_y_lower_left[1] - halo[1],
                    ]
                    x_y_upper_right = [
                        x_y_upper_right[0] + halo[2],
                        x_y_upper_right[1] + halo[3],
                    ]
                    x_y_lower_left_gcell[0] = bisect.bisect_left(
                        self.gcell_coordinate_x, x_y_lower_left[0]
                    )  # map to gcell grids
                    x_y_lower_left_gcell[1] = bisect.bisect_left(
                        self.gcell_coordinate_y, x_y_lower_left[1]
                    )
                    x_y_upper_right_gcell[0] = bisect.bisect_left(
                        self.gcell_coordinate_x, x_y_upper_right[0]
                    )
                    x_y_upper_right_gcell[1] = bisect.bisect_left(
                        self.gcell_coordinate_y, x_y_upper_right[1]
                    )
                    macro_map_with_halo[
                        x_y_lower_left_gcell[0] : x_y_upper_right_gcell[0],
                        x_y_lower_left_gcell[1] : x_y_upper_right_gcell[1],
                    ...
                    self.place_instance_dict[instance[1]] = [
                        instance[2],
                        (int(instance[l + 1]), int(instance[l + 2])),
                        instance[l + 4],
                    ]
        ...
        save(
            self.save_path, "macro_region", self.save_name, self.macro_map_with_halo
        )  # save macro_region feature
        read_file.close()
    ...
    def get_macro_region(self, line, x_y, area):  # function used by read_place_def
        ...
        x_y_lower_left_gcell = []
        x_y_upper_right_gcell = []
        x_y_lower_left_gcell.append(
            bisect.bisect_left(self.gcell_coordinate_x, x_y_lower_left[0])
        )
        x_y_lower_left_gcell.append(
            bisect.bisect_left(self.gcell_coordinate_y, x_y_lower_left[1])
        )
        direction = instance_direction_rect(line)
        x_y_upper_right = [
            x_y_lower_left[0] + area[0] * direction[0] + area[1] * direction[1],
            x_y_lower_left[1] + area[0] * direction[2] + area[1] * direction[3],
        ]
        x_y_upper_right_gcell.append(
            bisect.bisect_left(self.gcell_coordinate_x, x_y_upper_right[0])
        )
        x_y_upper_right_gcell.append(
            bisect.bisect_left(self.gcell_coordinate_y, x_y_upper_right[1])
        )
        return (
            x_y_lower_left_gcell,
            x_y_upper_right_gcell,
        ...
        for line in read_file:
            ...
            if line.startswith("COMPONENTS"):
                ...
                for i in range(len(GCELLY)):
                    ...
                    for i in range(top[1]):
                        self.gcell_coordinate_y.append(
                            self.gcell_coordinate_y[-1] + top[2]
                        )
                ...
                for i in range(len(GCELLX)):
                    top = GCELLX.pop()
                    for i in range(top[1]):
                        self.gcell_coordinate_x.append(
                            self.gcell_coordinate_x[-1] + top[2]
                        )
            ...
            if READ_GCELL:  # get gcell_coordinate
                ...
                if not len(instance) == 8:
                    ...
                    if "Y" in line:  # GCELLGRID Y 0 DO 2 STEP 190;
                        self.gcell_size[1] += int(instance[4])
                        GCELLY.append(gcell)

                    elif "X" in line:  # GCELLGRID X 0 DO 2 STEP 200;
            ...
            if READ_MACROS:  # get route_instance_dict
                if "FIXED" in line or "PLACED" in line:
                    instance = line.split()
                    l = instance.index("(")
                    self.route_instance_dict[instance[1].replace("\\", "")] = [
                        instance[2],
                        (int(instance[l + 1]), int(instance[l + 2])),
                        instance[l + 4],
                    ]
    ...
    def read_eGR_congestion(self):
            ...
            for line in read_file:
                if line.startswith("("):
                    ...
                    if int(capacity_vertical) == 0:
                        ...
                        self.congestion_vertical_overflow_eGR.append(
                            abs(overflow_vertical / capacity_vertical)
                        )
                        self.hotspot_eGR += 1
                    else:
                        self.congestion_vertical_util_eGR.append(
                            1 - overflow_vertical / capacity_vertical
                        )
                        self.congestion_vertical_overflow_eGR.append(0)
                    if int(capacity_horizontal) == 0:
                        ...
                        self.congestion_horizontal_overflow_eGR.append(
                            abs(overflow_horizontal / overflow_horizontal)
                        )
                        self.hotspot_eGR += 1
                    else:
                        self.congestion_horizontal_util_eGR.append(
                            1 - overflow_horizontal / capacity_horizontal
                        )
                        self.congestion_horizontal_overflow_eGR.append(0)
        first_resources = re.findall(r"\d+", first_line)
        gcell_size_eGR = [
            int(
                (int(resources[0]) - int(first_resources[0]))
                / (int(first_resources[2]) - int(first_resources[0]))
            )
            + 1,
            int(
                (int(resources[-5]) - int(first_resources[-5]))
                / (int(first_resources[3]) - int(first_resources[1]))
            )
            + 1,
        ]
        self.congestion_horizontal_util_eGR = np.array(
            self.congestion_horizontal_util_eGR
        ).reshape((gcell_size_eGR[0], gcell_size_eGR[1]), order="F")
        self.congestion_horizontal_overflow_eGR = np.array(
            self.congestion_horizontal_overflow_eGR
        ).reshape((gcell_size_eGR[0], gcell_size_eGR[1]), order="F")
        self.congestion_vertical_util_eGR = np.array(
            self.congestion_vertical_util_eGR
        ).reshape((gcell_size_eGR[0], gcell_size_eGR[1]), order="F")
        self.congestion_vertical_overflow_eGR = np.array(
            self.congestion_vertical_overflow_eGR
        ).reshape((gcell_size_eGR[0], gcell_size_eGR[1]), order="F")

        save(
            self.save_path,
            "congestion/congestion_early_global_routing/utilization_based/congestion_eGR_horizontal_util",
            self.save_name,
            self.congestion_horizontal_util_eGR,
        )
        save(
        ...
        save(
            self.save_path,
            "congestion/congestion_early_global_routing/utilization_based/congestion_eGR_vertical_util",
            self.save_name,
            self.congestion_vertical_util_eGR,
        )
        save(
            ...
            for line in read_file:
                if line.startswith("("):
                    ...
                    if int(capacity_vertical) == 0:
                        ...
                        self.congestion_vertical_overflow.append(
                            -overflow_vertical / capacity_vertical
                        )
                        self.hotspot += 1
                    else:
                        self.congestion_vertical_util.append(
                            1 - (used_vertical / capacity_vertical)
                        )
                        self.congestion_vertical_overflow.append(0)
                    if int(capacity_horizontal) == 0:
                        ...
                        self.congestion_horizontal_overflow.append(
                            -overflow_horizontal / capacity_horizontal
                        )
                        self.hotspot += 1
                    else:
                        self.congestion_horizontal_util.append(
                            1 - (used_horizontal / capacity_horizontal)
                        )
                        self.congestion_horizontal_overflow.append(0)
                    self.gcell_coordinate_x.append(int(resources[2]))
                    self.gcell_coordinate_y.append(int(resources[3]))

        self.congestion_horizontal_util = np.array(
            self.congestion_horizontal_util
        ).reshape((self.gcell_size[0], self.gcell_size[1]), order="F")
        self.congestion_horizontal_overflow = np.array(
            self.congestion_horizontal_overflow
        ).reshape((self.gcell_size[0], self.gcell_size[1]), order="F")
        self.congestion_vertical_util = np.array(self.congestion_vertical_util).reshape(
            (self.gcell_size[0], self.gcell_size[1]), order="F"
        )
        self.congestion_vertical_overflow = np.array(
            self.congestion_vertical_overflow
        ).reshape((self.gcell_size[0], self.gcell_size[1]), order="F")
        self.gcell_coordinate_x = np.array(self.gcell_coordinate_x).reshape(
            (self.gcell_size[0], self.gcell_size[1]), order="F"
        )[..., 0]
        self.gcell_coordinate_y = np.array(self.gcell_coordinate_y).reshape(
            (self.gcell_size[0], self.gcell_size[1]), order="F"
        )[0, ...]
        assert (
            self.gcell_coordinate_x.size == self.gcell_size[0]
            and self.gcell_coordinate_y.size == self.gcell_size[1]
        )

        save(
            self.save_path,
            "congestion/congestion_global_routing/utilization_based/congestion_GR_horizontal_util",
            self.save_name,
            self.congestion_horizontal_util,
        )
        save(
                ...
                if READ_MACROS:
                    if "FIXED" in line or "PLACED" in line:
                        ...
                        instance_coord = [
                            ...
                            float(
                                int(line[left + 1])
                                + instance_size[0] * instance_direction[0]
                                + instance_size[1] * instance_direction[1]
                            ),
                            float(
                                int(line[left + 2])
                                + instance_size[0] * instance_direction[2]
                                + instance_size[1] * instance_direction[3]
                            ),
                        ]

                        out_instance_dic_micron[instance_name] = [
                            x * self.scaling / self.unit for x in instance_coord
                        ]

                        instance_coord_gcell = [
                            bisect.bisect_left(
                                self.gcell_coordinate_x, instance_coord[0]
                            ),
                            bisect.bisect_left(
                                self.gcell_coordinate_y, instance_coord[1]
                            ),
                            bisect.bisect_left(
                                self.gcell_coordinate_x, instance_coord[2]
                            ),
                            bisect.bisect_left(
                                self.gcell_coordinate_y, instance_coord[3]
                            ),
                        ]
    ...
    def read_route_pin_position(self):
                ...
                for line in read_file:
                    ...
                    if READ_MACROS:
                        if "FIXED" in line or "PLACED" in line:
                            ...
                            self.route_instance_dict[instance[1].replace("\\", "")] = [
                                ...
                                (int(instance[l + 1]), int(instance[l + 2])),
                                instance[l + 4],
                            ]  # key: inst's name; value: 1. name of std cell 2. location of inst 3. direction of inst
                    ...
                    if READ_PINS:
                        if line.startswith("-"):
                            ...
                            data = line.split()
                            self.route_pin_dict[pin]["location"] = [
                                int(data[3]),
                                int(data[4]),
                            ]
                            self.route_pin_dict[pin]["direction"] = data[6]
        ...
        for cell_name, cell_features in route_instance_dict.items():
            ...
            for pin_name, pin_coordinate in self.lef_dict[cell_features[0]][
                "pin"
            ].items():  # pin_coordinate left/lower/right/upper
                direction = instance_direction_bottom_left(cell_features[2])
                std_cell_x, std_cell_y = self.lef_dict[cell_features[0]]["size"]
                instance_location_on_chip[0] = (
                    cell_features[1][0]
                    + direction[8] * std_cell_x
                    + direction[9] * std_cell_y
                )
                instance_location_on_chip[1] = (
                    cell_features[1][1]
                    + direction[10] * std_cell_x
                    + direction[11] * std_cell_y
                )

                pin_left = (
                    instance_location_on_chip[0]
                    + pin_coordinate[0] * direction[4]
                    + pin_coordinate[1] * direction[5]
                    + pin_coordinate[2] * direction[0]
                    + pin_coordinate[3] * direction[1]
                )
                pin_lower = (
                    instance_location_on_chip[1]
                    + pin_coordinate[0] * direction[6]
                    + pin_coordinate[1] * direction[7]
                    + pin_coordinate[2] * direction[2]
                    + pin_coordinate[3] * direction[3]
                )
                pin_right = (
                    instance_location_on_chip[0]
                    + pin_coordinate[2] * direction[4]
                    + pin_coordinate[3] * direction[5]
                    + pin_coordinate[0] * direction[0]
                    + pin_coordinate[1] * direction[1]
                )
                pin_upper = (
                    instance_location_on_chip[1]
                    + pin_coordinate[2] * direction[6]
                    + pin_coordinate[3] * direction[7]
                    + pin_coordinate[0] * direction[2]
                    + pin_coordinate[1] * direction[3]
                )
                pin_position_micron = [
                    round(pin_left * self.scaling / self.unit, 2),
                    round(pin_lower * self.scaling / self.unit, 2),
                    round(pin_right * self.scaling / self.unit, 2),
                    round(pin_upper * self.scaling / self.unit, 2),
                ]
                ...
                pin_position_dict["%s/%s" % (cell_name, pin_name)].extend(
                    pin_position_gcell
                )
    ...
    def compute_density(self, density, location_on_gcell):
        gcell_left, gcell_lower, gcell_right, gcell_upper = (
            location_on_gcell  # 在计算drc密度时 drc的x1 y1 x2 y2
        )
        if (
            gcell_left == self.gcell_coordinate_x.size
        ):  # avoid getting size+1 from bisect_left
        ...
        for j in my_range(gcell_left, gcell_right):  # 同正常的range
            for k in my_range(gcell_lower, gcell_upper):
                density[
                    j, k
                ] += 1  # 所以这里只是在gcell size大小的区域内，对应drc的位置+1吗
        return density

    def compute_density_with_overlap(
        self, density, location_on_coordinate, location_on_gcell
    ):
        ...
        for j in my_range(gcell_left, gcell_right):
            for k in my_range(gcell_lower, gcell_upper):
                ...
                if (right - left) * (upper - lower) == 0:
                    print(right, left)
                overlap = (
                    (min(right, x_right) - max(left, x_left))
    ...
    def compute_cell_density(self):
        ...
        for n in self.place_instance_dict:
            ...
            cell_y_lower_gcell = bisect.bisect_left(
                self.gcell_coordinate_y, instance[1][1]
            )
            cell_size = self.lef_dict[instance[0]]["size"]
            direction = instance_direction_rect(instance[2])
            cell_x_right = (
                instance[1][0]
                + cell_size[0] * direction[0]
                + cell_size[1] * direction[1]
            )
            cell_y_upper = (
                instance[1][1]
                + cell_size[0] * direction[2]
                + cell_size[1] * direction[3]
            )
            cell_x_right_gcell = bisect.bisect_left(
                self.gcell_coordinate_x, cell_x_right
            )
            cell_y_upper_gcell = bisect.bisect_left(
                self.gcell_coordinate_y, cell_y_upper
            )
            cell_coordinate = [
                cell_x_left_gcell,
                cell_y_lower_gcell,
    ...
    def get_RUDY(self):
        ...
        for net in self.place_net_dict.keys():
            ...
            for cell_pin_pair in self.place_net_dict[
                net
            ]:  # e.g. axi_interconnect_i/axi_node_i/_RESP_BLOCK_GEN\[1\].RESP_BLOCK/BR_ALLOC/U70 I cell pin pair
                instance_location_on_chip = [0, 0]

                if cell_pin_pair[0] == "PIN":
                    pin_left, pin_right, pin_lower, pin_upper = self.place_pin_dict[
                        cell_pin_pair[1]
                    ]
                else:
                    cell_name = cell_pin_pair[0]
                    pin_name = cell_pin_pair[1]
                    direction = instance_direction_bottom_left(
                        self.place_instance_dict[cell_name][2]
                    )
                    std_cell_x, std_cell_y = self.lef_dict[
                        self.place_instance_dict[cell_name][0]
                    ]["size"]
                    # convert coordinate to lower left corner
                    instance_location_on_chip[0] = (
                        self.place_instance_dict[cell_name][1][0]
                        + direction[8] * std_cell_x
                        + direction[9] * std_cell_y
                    )
                    instance_location_on_chip[1] = (
                        self.place_instance_dict[cell_name][1][1]
                        + direction[10] * std_cell_x
                        + direction[11] * std_cell_y
                    )

                    # pin_location_on_instance left/lower/right/upper
                    pin_location_on_instance = self.lef_dict[
                        self.place_instance_dict[cell_name][0]
                    ]["pin"][pin_name]
                    pin_left = (
                        instance_location_on_chip[0]
                        + pin_location_on_instance[0] * direction[4]
                        + pin_location_on_instance[1] * direction[5]
                        + pin_location_on_instance[2] * direction[0]
                        + pin_location_on_instance[3] * direction[1]
                    )
                    pin_lower = (
                        instance_location_on_chip[1]
                        + pin_location_on_instance[0] * direction[6]
                        + pin_location_on_instance[1] * direction[7]
                        + pin_location_on_instance[2] * direction[2]
                        + pin_location_on_instance[3] * direction[3]
                    )
                    pin_right = (
                        instance_location_on_chip[0]
                        + pin_location_on_instance[2] * direction[4]
                        + pin_location_on_instance[3] * direction[5]
                        + pin_location_on_instance[0] * direction[0]
                        + pin_location_on_instance[1] * direction[1]
                    )
                    pin_upper = (
                        instance_location_on_chip[1]
                        + pin_location_on_instance[2] * direction[6]
                        + pin_location_on_instance[3] * direction[7]
            ...
            if pin_location_on_chip_x == []:
                ...
                temp_rudy, long_or_short = self.compute_RUDY(
                    temp_rudy, location_on_coordinate, location_on_gcell
                )
                RUDY += temp_rudy
                pin_RUDY_weight = 1 / (net_right - net_left) + 1 / (
                    net_upper - net_lower
                )
                if (
                    long_or_short == "long"
                ):  # short: cover only 1 Gcell, long: cover more than 1
                    RUDY_long += temp_rudy
    ...
    def compute_RUDY(self, rudy, location_on_coordinate, location_on_gcell):
        ...
        rudy = weight * self.compute_density_with_overlap(
            rudy, location_on_coordinate, location_on_gcell
        )
        ...
        with open(self.drc_path, "r") as read_file:
            for line in read_file:
                if "Net" in line or "Cell" in line:
                    ...
                    # print(drc_area)
                    self.drc_dict[drc_type][drc_layer].append(
                        [
                            float(drc_area[0]) * self.unit,
                            float(drc_area[1]) * self.unit,
                            float(drc_area[2]) * self.unit,
                            float(drc_area[3]) * self.unit,
                        ]
                    )
                else:
                    pass
    ...
    def compute_DRC_density(self):
        ...
        for types in self.drc_dict:  # first layer of dict: key types
            ...
            save_dir = os.path.join(
                save_path, "DRC_seperated", types.replace(" ", ""), self.save_name
            )
            os.system("mkdir -p %s " % (os.path.dirname(save_dir)))
            for layer in self.drc_dict[types]:  # second: metal layer
                for drc in self.drc_dict[types][
                    layer
                ]:  # drc_dict[types][layer] third: coordinate
                    drc_left, drc_lower, drc_right, drc_upper = drc
                    # 查找 drc_left 在 self.gcell_coordinate_x 中的位置（索引），并返回该位置。
                    # 这个位置是 drc_left 插入 self.gcell_coordinate_x 时，保持列表有序的地方。
                    drc_x_left_gcell = bisect.bisect_left(
                        self.gcell_coordinate_x, drc_left
                    )
                    drc_y_lower_gcell = bisect.bisect_left(
                        self.gcell_coordinate_y, drc_lower
                    )
                    drc_x_right_gcell = bisect.bisect_left(
                        self.gcell_coordinate_x, drc_right
                    )
                    drc_y_upper_gcell = bisect.bisect_left(
                        self.gcell_coordinate_y, drc_upper
                    )
                    drc_gcell = [
                        drc_x_left_gcell,
                        drc_y_lower_gcell,
                    ...
                    drc_density_dict[types] = self.compute_density(
                        drc_density_dict[types], drc_gcell
                    )
                    type_count += 1
                    overall_count += 1
                ...
                if start:
                    ...
                    if read:
                        if len(line.split()) == 1:
                            ...
                            if (
                                line.split()[-1] not in self.lef_dict
                                or not self.lef_dict[line.split()[-1]]["type"]
                                == "std_cell"
                            ):
                                continue
                            data = line.split()
    ...
    def get_power_map(self):
        ...
        for k, v in self.power_dict.items():
            ...
            cell_x_right = (
                cell_x_left + cell_size[0] * direction[0] + cell_size[1] * direction[1]
            )
            cell_y_upper = (
                cell_y_lower + cell_size[0] * direction[2] + cell_size[1] * direction[3]
            )
            cell_x_left_gcell = bisect.bisect_left(self.gcell_coordinate_x, cell_x_left)
            cell_y_lower_gcell = bisect.bisect_left(
                self.gcell_coordinate_y, cell_y_lower
            )
            cell_x_right_gcell = bisect.bisect_left(
                self.gcell_coordinate_x, cell_x_right
            )
            ...
            power_map += self.compute_density_with_overlap(
                tmp_power_map, location, location_gcell
            )
            n_pin = len(tw)
            ...
            if tw:
                for i in tw:
                    if i == 0:
                        ...
                        self.power_t[i[0] : i[1] + 1, :, :] += power_map * (
                            (v[1] + v[2]) * v[0] + v[3]
                        )
            ...
            for line in read_file:
                ...
                if read:
                    ...
                    if not data[1] == "M4":
                        first_flag = True
                    if first_flag:
                        gcell_x = bisect.bisect_left(
                            self.gcell_coordinate_x, float(data[2]) * self.unit
                        )
                        gcell_y = bisect.bisect_left(
                            self.gcell_coordinate_y, float(data[3]) * self.unit
                        )
                        if float(data[0]) > self.ir_map[gcell_x, gcell_y]:
                            self.ir_map[gcell_x, gcell_y] = float(
                                data[0]
                            )  # v to mv *1000  % /100
    ...
    def get_pin_configuration_map(self):
        ...
        pin_map_M1 = np.zeros(
            (
                round(self.gcell_coordinate_x[-1] / scale),
                round(self.gcell_coordinate_y[-1] / scale),
            ),
            dtype=np.int8,
        )
        pin_map_M2 = np.zeros(
        ...
        for v in self.route_instance_dict.values():
            ...
            instance_location_on_chip[0] += (
                direction[8] * std_cell_x + direction[9] * std_cell_y
            )  # 先把坐标转回原本的左下点
            instance_location_on_chip[1] += (
                direction[10] * std_cell_x + direction[11] * std_cell_y
            )
            ...
            for pin_name, pin_data in lef_dict["pin"].items():
                ...
                for layer, rects in pin_data.items():
                    ...
                    for pin in rects:
                        ...
                        if layer == "M1":
                            pin_map_M1[
                                pin_left : (pin_right + 1), pin_lower : (pin_upper + 1)
                            ] = 1
                        else:
                            pin_map_M2[
                                pin_left : (pin_right + 1), pin_lower : (pin_upper + 1)
...
if __name__ == "__main__":
    lef_dir = os.path.join(os.path.dirname(__file__), "../LEF")
    cells_lef = os.path.join(lef_dir, "cells.lef")

    lef_dict = {}
    lef_dict = read_lef(cells_lef, lef_dict, 1000)
