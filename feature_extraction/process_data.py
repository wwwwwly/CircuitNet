...
class Paraser(object):
    def __init__(self) -> None:
        ...
        self.parser.add_argument(
            "--data_root", default="./data", help="the parent dir of innovus workspace"
        )
        self.parser.add_argument(
            "--lef_path", default=["./LEF/circuitnet.lef"], help="path to LEF files"
        )  # 这里的默认路径是一个列表，所以后面能遍历
        self.parser.add_argument(
            "--unit", default=2000, help="unit defined in the begining of DEF"
        )
        self.parser.add_argument("--save_path", default="./out", help="save path")
        self.parser.add_argument(
            "--process_capacity",
            default=10,
            help="multi process setting, number of files for each process, determine number of process",
        )
        self.parser.add_argument(
            "--debug", default=False, help="disable multi process to use pdb"
        )
        ...
        self.parser.add_argument(
            "--eGR_congestion_name", default="eGR_congestion"
        )  # congestion report from 'dumpNanoCongestArea'
        self.parser.add_argument("--route_congestion_name", default="route_congestion")
        self.parser.add_argument(
            "--drc_rpt_name", default="drc.rpt"
        )  # drc report from 'verify_drc'
        self.parser.add_argument(
            "--twf_rpt_name", default="cts.twf"
        )  # timing window file from 'write_timing_windows'
        self.parser.add_argument(
            "--power_rpt_name", default="dyn_power.rpt"
        )  # power report from 'report_power'
        self.parser.add_argument(
            "--ir_rpt_name", default="route_dynamic_ir.rpt"
        )  # ir report form 'report_power_rail_results'
        self.parser.add_argument(
            "--n_time_window", default="20"
        )  # number of divided timing windows
