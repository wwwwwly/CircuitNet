...
class conv(nn.Module):
    def __init__(self, dim_in, dim_out, kernel_size=3, stride=1, padding=1, bias=True):
        ...
        self.main = nn.Sequential(
            ...
            nn.BatchNorm2d(dim_out),  # gpdl中的卷积块 使用的是InstanceNorm2d
...
class Encoder(nn.Module):
    def __init__(self, in_dim=3, out_dim=32):
        ...
        self.c3 = nn.Sequential(
            nn.Conv2d(64, out_dim, 3, 1, 1), nn.BatchNorm2d(out_dim), nn.Tanh()
        )
...
class Decoder(nn.Module):
    def __init__(self, out_dim=2, in_dim=32):
        ...
        self.conv3 = nn.Sequential(nn.Conv2d(4, out_dim, 3, 1, 1), nn.Sigmoid())  # original
        # self.conv3 = nn.Sequential(nn.Conv2d(4, out_dim, 3, 1, 1))  # test 1 no sigmoid
        # self.conv3 = nn.Sequential(nn.Conv2d(4, out_dim, 3, 1, 1), nn.LeakyReLU())  # test 2
